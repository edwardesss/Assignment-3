...
function App() {
  ...
  function onDeleteUser(user){
    let del = window.confirm("Do you really want to delete the task for   " + user.name);
    if(del){
      axios.delete('https://omar-3faaf-default-rtdb.firebaseio.com/users/'+user.id+'.json')
      .then((response) => {
        console.log(response);
        fetchUsers(); 
  ...
  return (
      <div>
        ...
        {!loading && <UserDetails users = {users} onEditUser={onEditUser} onDeleteUser={onDeleteUser}></UserDetails>}
